...
from pypesq import pesq as pypesq# https://github.com/ludlows/python-pesq
...
def rev_llr(sig_cln, sig_proc, fs, frameLen=0.03, overlap=0.75):
    stop = len(sig_cln)
    ...
    for i in range(1, stop):
        cur_s2 = stop - i
        if sig_proc[cur_s2] != 0:
            break
    cur_s = min([cur_s1, cur_s2])
    while True:
        try:
            cur_l = llr(sig_cln[:cur_s], sig_proc[:cur_s], fs, frameLen, overlap)
            return cur_l
        except:
            cur_s -= 1000
            if cur_s <= 0:
...
if __name__ == '__main__':
    import librosa
    filename =\
    '/data/machao/jiaxp/datasets/wsj0_reverb_v2/infer/reverb/050a0501.wav'
    sig_r, _ = librosa.load(filename, 8000)
    filename = filename.replace('/reverb', '/direct')
    sig_d, _ = librosa.load(filename, 8000)
    print(sig_r.shape)
    stop = len(sig_r)
    for i in range(1, len(sig_r)):
        cur_s = stop - i
        if sig_d[cur_s] != 0:
            print(cur_s)
            break

    print(rev_llr(sig_d, sig_r, 8000))
    sig_r = sig_r[:35367]
    sig_d = sig_d[:35367]
    print(llr(sig_d, sig_r, 8000))
