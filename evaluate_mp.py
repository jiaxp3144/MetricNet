...
FLAG_LLR = False
FLAG_SRMR = False
RESULT_PATH = './log_2s_4GPUs_predRvb/test/'
RESULT_FILE = os.path.join(RESULT_PATH, '../result_mp.txt')
WORKER_NUM = 8
...
def evaluator(cur_path, cur_index, total_index):
    cur_res_file = os.path.join(cur_path, 'enh.wav')
    cur_cln_file = os.path.join(cur_path, 'cln.wav')
    cur_rvb_file = os.path.join(cur_path, 'mix.wav')
    [fs, sig_res] = wavfile.read(cur_res_file)
    [fs, sig_cln] = wavfile.read(cur_cln_file)
    [fs, sig_rvb] = wavfile.read(cur_rvb_file)
    if len(sig_rvb) != len(sig_cln):
    ...
    res_array = np.zeros(12)

    cur_str = "{} / {}: ".format(cur_index, total_index)

    if FLAG_SDR:
        res_array[1] = cur_sdr = calcSDR(sig_cln, sig_res)
        res_array[0] = cur_sdr_r = calcSDR(sig_cln, sig_rvb)
        cur_str += "sdr: {:.3f}dB --> {:.3f}dB ".format(cur_sdr_r, cur_sdr)
    if FLAG_SISDR:
        res_array[3] = cur_sisdr = calcSISDR(sig_cln, sig_res)
        res_array[2] = cur_sisdr_r = calcSISDR(sig_cln, sig_rvb)
        cur_str += "sisdr: {:.3f}dB --> {:.3f}dB ".format(cur_sisdr_r, cur_sisdr)
    if FLAG_PESQ:
        res_array[5] = cur_pesq = calcPesq(sig_cln, sig_res, fs)
        res_array[4] = cur_pesq_r = calcPesq(sig_cln, sig_rvb, fs)
        cur_str += "pesq: {:.3f} --> {:.3f} ".format(cur_pesq_r, cur_pesq)
    if FLAG_STOI:
        res_array[7] = cur_stoi = calcStoi(sig_cln, sig_res, fs)
        res_array[6] = cur_stoi_r = calcStoi(sig_cln, sig_rvb, fs)
        cur_str += "stoi: {:.3f} --> {:.3f} ".format(cur_stoi_r, cur_stoi)
    if FLAG_LLR:
        res_array[9] = cur_llr = calcLLR(sig_cln, sig_res, fs)
        res_array[8] = cur_llr_r = calcLLR(sig_cln, sig_rvb, fs)
        if cur_llr == -1 or cur_llr_r == -1:
            res_array[9] = cur_llr = np.nan
            res_array[8] = cur_llr_r = np.nan
        cur_str += "llr: {:.3f} --> {:.3f} ".format(cur_llr_r, cur_llr)
    if FLAG_SRMR:
        cur_srmr, _ = calcSRMR(sig_res, fs, fast=True, norm=True)
        cur_srmr_r, _ = calcSRMR(sig_rvb, fs, fast=True, norm=True)
        cur_str += "srmr: {:.3f} --> {:.3f} ".format(cur_srmr_r, cur_srmr)
        res_array[11] = cur_srmr
        res_array[10] = cur_srmr_r
              
    print(cur_str)
    return [cur_index, res_array, cur_str]


def evaluate_mp(result_path, result_file):
    dir_list = os.listdir(result_path)
    file_num = len(dir_list)

    p = Pool(WORKER_NUM)
    result = []
    for cur_index in range(file_num):
        cur_dir = dir_list[cur_index]
        cur_path = os.path.join(result_path, cur_dir)
        result.append(p.apply_async(func=evaluator, args=(cur_path, cur_index,
                                                          file_num,)))
    p.close()
    p.join()
    ...
    results.sort()
    res_array = np.zeros([file_num, 12])
    f = open(result_file, 'w')
    for line in results:
        cur_index, cur_array, cur_str = line
        f.write(cur_str + '\n')
        res_array[cur_index, :] = cur_array
    final_res = np.nanmean(res_array, axis=0)
    
    cur_str = '\nsummary: '
    if FLAG_SDR:
        cur_str += 'sdr = {:.3f}dB --> {:.3f}dB, '.format(final_res[0],
                                                          final_res[1])
    if FLAG_SISDR:
        cur_str += 'sisdr = {:.3f}dB --> {:.3f}dB, '.format(final_res[2],
                                                            final_res[3])
    if FLAG_PESQ:
        cur_str += 'pesq = {:.3f} --> {:.3f}, '.format(final_res[4],
                                                       final_res[5])
    if FLAG_STOI:
        cur_str += 'stoi = {:.3f} --> {:.3f} '.format(final_res[6],
                                                      final_res[7])
    if FLAG_LLR:
        cur_str += 'llr = {:.3f} --> {:.3f} '.format(final_res[8], final_res[9])
    if FLAG_SRMR:
        cur_str += 'srmr = {:.3f} --> {:.3f} '.format(final_res[10],
                                                      final_res[11])
    print(cur_str)
    f.write(cur_str + '\n')
    f.close()
